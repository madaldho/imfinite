...
const items = [
  ...
  { id: 2, img: "/img/cat.webp", title: "cat wearing a  crown", prompt: "Ragdoll cat king wearing a golden crown, intricate, elegant, highly detailed" },
        ...
        <div data-aos="fade-in" className="flex grid-cols-3 justify-center gap-3 px-2">
          {items.map((i, idx) => (
            <div className="flate" key={i.id}>
              <button
                onClick={() => setDemo(idx)}
                className="group mx-auto block max-w-xs space-y-2 rounded-full bg-slate-400/40 p-2 shadow-lg ring-1 ring-slate-900/5 hover:bg-white/30 hover:ring-white md:pl-6"
              >
